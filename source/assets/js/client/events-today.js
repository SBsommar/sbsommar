...
(function () {
  ...
  // on that day. A cross-day move always keeps its ghost. The threshold has a
  // single home in the shared ghost-config.js (SBGhostConfig), loaded before this
  // script; if it is somehow missing we keep the ghost rather than invent a value.
  function suppressGhost(e) {
    if (e.moved.from_date !== e.date) return false;
    if (!window.SBGhostConfig) return false;
    var max = window.SBGhostConfig.GHOST_MAX_ACTIVITIES_BETWEEN;
