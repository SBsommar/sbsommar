...
// ── §96.1 — Cache name is sb-sommar-v9 ──────────────────────────────────────

describe('02-§96.1 — Cache name is sb-sommar-v9', () => {
  it('OFF-02: sw.js uses sb-sommar-v9 cache name', () => {
    const swPath = path.join(__dirname, '..', 'source', 'static', 'sw.js');
    const src = fs.readFileSync(swPath, 'utf8');
    assert.ok(
      src.includes("'sb-sommar-v9'"),
      'CACHE_NAME must be sb-sommar-v9',
    );
  });
});
