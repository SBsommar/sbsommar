...
| `05-§5.2` | Uniqueness of `(title + date + start)` holds across the camp file **and** its fragment files together | 03-architecture/data-layer.md §1.1; 02-requirements/event-data.md §109 | FRAG-05, FRAG-38 | Deterministic id → identical fragment file name; add API rejects a duplicate create with HTTP 422 (02-§109.8); `loadCampEvents()` de-dups so the rendered set stays unique (02-§109.20) | implemented |
| `05-§6.1` | Event `id` must be unique within the camp file and, when fragments are used, across the camp file and all its fragments | 06-EVENT_DATA_MODEL.md §4; 03-architecture/data-layer.md §1.1 | GH-01..11 (slugify determinism), LNT-18, FRAG-04 | `source/scripts/lint-yaml.js` – `seenIds` set (build-time + CI); `source/build/load-events.js` – `loadCampEvents()` enforces id==filename + de-dups; API generates deterministic IDs | covered |
