...
| MOVED-01..51 | `tests/moved-activity.test.js` | Moved + relocated capture, serialisation, validation, helpers, schedule/event-page markup + ghost, ghost suppression for nearby same-day moves, and the desired-state styling (plain new time, amber marker on the old time, smaller ghost row) (02-§119) |
| CLASH-01..22 | `tests/location-clash.test.js` | Location-clash marking: later room-chooser flagged (by `location_set_at`, `created_at` fallback), Annat/cancelled/back-to-back excluded, `location_set_at` maintenance, `is-clash` markup (02-§120) |
