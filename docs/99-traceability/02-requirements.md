...
| `02-§18.3` | The session cookie has Max-Age of 180 days; every activity (add or edit) re-signs all held entries and refreshes the cookie lifetime | 03-architecture/forms-and-api.md §7, §7.1 | SES-07, SES-21, SES-24 | `createOwnershipEntry()` signs a 180-day horizon; add and edit handlers re-sign `parseHealableSessionIds()` entries before writing `Max-Age=15552000` cookie | covered |
| `02-§18.51` | Ownership self-heals: an authentic entry past its horizon still authorizes editing/deleting a future event and is re-signed on the next activity; past events stay blocked | 03-architecture/forms-and-api.md §7.1 | SES-22, SES-23, SES-24, SES-25, SES-26 | `parseHealableSessionIds()`/`verifyOwnershipSignature()` accept authentic-but-expired entries; edit/delete handlers use them and keep the independent past-event check | covered |
...
| `02-§18.5` | The session cookie is JavaScript-readable (not httpOnly); server-side validation of signed ownership compensates | 03-architecture/forms-and-api.md §7 | EEC-26, SES-17..21 | `buildSetCookieHeader()` omits HttpOnly; edit/delete authorization verifies signatures server-side via `parseHealableSessionIds()` | covered |
...
| `02-§26.6` | After last camp day (`today > end_date`): add-activity form greyed out (reduced opacity) | 03-architecture/forms-and-api.md §13.2, §13.3 | GATE-02, RADD-23 (add form `data-closes = end_date`); — (manual: open add form the day after `end_date`, confirm fields greyed out) | `source/build/render-add.js` – `closesDate = endDate`; `source/assets/js/client/lagg-till.js` – sets `fieldset.disabled` and `form-gated` class when `today > closes` | implemented |
| `02-§26.7` | After last camp day: submit button disabled | 03-architecture/forms-and-api.md §13.3 | — (manual: open form after `end_date`, confirm submit disabled) | `source/assets/js/client/lagg-till.js` – `submitBtn.disabled = true` | implemented |
| `02-§26.8` | After last camp day: message "Lägret är avslutat. Det går inte längre att lägga till aktiviteter." | 03-architecture/forms-and-api.md §13.3 | — (manual: open add form the day after `end_date`, confirm message) | `source/assets/js/client/lagg-till.js` – inserts `.form-gate-msg` with the message | implemented |
| `02-§26.9` | Before-opening rules apply identically to edit-activity form | 03-architecture/forms-and-api.md §13.3 | — (manual: open edit form before `opens_for_editing`, confirm gating behaviour) | `source/assets/js/client/redigera.js` – time-gate check using `data-opens` / `data-closes` | implemented |
...
| `02-§26.13` | Build embeds `data-opens` (= `opens_for_editing`) and `data-closes` (add form: `end_date`; edit form: `end_date + 1`) on form | 03-architecture/forms-and-api.md §13.2 | GATE-01..04, RADD-22..24, REDT-22..24 | `source/build/render-add.js` (`data-closes = end_date`), `source/build/render-edit.js` (`data-closes = end_date + 1`) | covered |
...
| `02-§26.21` | Edit form locks after `end_date + 1 day` with "Lägret är avslutat." (full server period); add-form lock does not apply | 03-architecture/forms-and-api.md §13.2, §13.3 | GATE-04 (edit form `data-closes = end_date + 1`); — (manual: open edit form the day after `end_date`, confirm still open; after `end_date + 1`, confirm locked) | `source/build/render-edit.js` – `closesDate = addOneDay(endDate)`; `source/assets/js/client/redigera.js` – `today > closesDate` check | implemented |
...
| `02-§80.4` | manual | Day grid never offers a day before today: all days before camp, today-onward during camp, none after the last day (browser-only; `lagg-till.js` hides `btn.dataset.date < today`) |
...
| `02-§101.8` | covered | SES-04, SES-05, SES-19, SES-20, SES-23, SES-25: malformed, legacy, wrong-secret, and tampered entries are ignored; SES-22, SES-24 confirm an authentic entry past its horizon is honoured (self-healing) rather than rejected |
...
| `02-§101.14` | covered | EEC-19, SES-07, SES-24: a successful edit re-signs every authentic entry (`parseHealableSessionIds` → `createOwnershipEntry`) and returns a refreshed 180-day cookie; `app.js`/`api/index.php` `edit-event` handlers set `Set-Cookie` on success |
...
| `02-§119.13` | covered | MOVED-23/-24: the previous time is real struck-through text and the `Flyttad till` label is real text; amber (`.ev-time-old`) is an additional cue. CSS contrast is a manual checkpoint |
